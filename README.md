...
#### GIthub OAuthアプリ
GitHub OAuth アプリを登録しなければならない。 **Callback URL** に以下を設定しなければならない。
```
https://coder.v2v.chip-in.net/api/v2/users/oauth2/github/callback
https://auth.v2v.chip-in-v2.net/oauth2/callback
```
...
|GH_CLIENT_ID|Github ClientID||必須|
|GH_CLIENT_SECRET|Github Secret||必須|
...
### Coder テンプレートの登録
Coder にログインしてセッショントークンを作成後：
```bash
CODER_SESSION_TOKEN=<token> ./deploy.sh
```
